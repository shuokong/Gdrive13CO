...
imagein = 'convol28_mask_imfit_13co_pix_2_Tmb.fits'
...
coresample = [99,655,282,357,294,610,255,309]
...
for n,i in enumerate(corenums):
    ...
    if i not in coresample: continue
...
subxw = []
subyw = []
os.system('rm -r Lane_cores_smoothed_spectra')
os.system('mkdir Lane_cores_smoothed_spectra')
for counter,coresubgroup in enumerate(coregroups):
    ...
    for nn,ii in enumerate(coresubgroup):
        ...
        ff,corenum,rank = ii
        corei = np.array(ff) 
        region = 'Lane'+str(int(corenum)).zfill(3) 
        ...
        if nn+1 == 1:
            plt.title(r'$^{13}$CO(1-0)',fontsize=20)
        ...
        savetxtarr = np.stack((velocity,spectrum),axis=1)
        np.savetxt('Lane_cores_smoothed_spectra/13CO_'+region+'.txt',savetxtarr,fmt='%.2f %.2f')
    pdfname = 'Lane_cores_smoothed_spectra/averspec13_'+region+'.pdf'
