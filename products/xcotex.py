...
hdulist1=pyfits.open('regrid_Stutz_convol18_mom0_13co_pix_2_Tmb.fits')
...
mom013=hdulist1[0].data[0,:,:]
mom0rms = 1.1 # K km/s
nicest=hdulist2[0].data[:,:]
meanxcousedata = (~np.isnan(mom013))&(nicest>0)&(mom013>3*mom0rms)
tex=hdulist3[0].data[:,:]
av = nicest/9.4e20/2.

print 'average NH',np.nanmean(nicest[meanxcousedata]),'average mom0',np.nanmean(mom013[meanxcousedata]),'mean Xco',np.nanmean(nicest[meanxcousedata])/np.nanmean(mom013[meanxcousedata])

hdulist1.close()
hdulist2.close()
hdulist3.close()

rawxco = nicest/mom013
hdulist4[0].data = nicest/mom013
...
def plothist(data,pdfname):
    ...
    #plt.xscale('log')
    #plt.yscale('log')
    plt.xlabel(r'$X_{\rm ^{13}CO}~({\rm cm}^{-2}~{\rm (K~km~s^{-1})}^{-1})$')
    plt.ylabel('counts')
    plt.xlim(0,4.e21)
    plt.grid(True)
...
#            if mom013[i,j] > 0 and nicest[i,j] > 0 and tex[i,j] > 0:
#                log_xco.append(math.log(nicest[i,j]*9.4e20/(mom013[i,j]/0.7),10))
#                log_tex.append(math.log(tex[i,j],10))

usedata = (tex>2.75) & (~np.isnan(tex)) & (av>3)
nan_log_xco = np.log10(nicest/mom013)
